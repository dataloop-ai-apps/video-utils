...
class ByteTrackTracker(BaseTracker):
    ...
    def update(self, frame, fn, frame_item, video_item):
        ...
        input_boxes = []  # NEW: (left, top, right, bottom, label, ann object)
        ...
        for i, ann in enumerate(frame_annotation):
            ...
            input_boxes.append((t, l, b, r, getattr(ann, "label", None), ann))  # NEW
        ...
        for t in online_targets:
            print(f"tlwh: {t.tlwh} , tlbr: {t.tlbr} , track_id: {t.track_id}")
            tlwh = t.tlwh
            ...
            for t, l, b, r, label, ann in input_boxes:
                curr_iou = iou((t, l, b, r), tlbr)
                if curr_iou > best_iou:
                    best_iou = curr_iou
                    best_label = label
            # You can set a threshold, e.g. best_iou > 0.5 if you want.

            # MODIFIED: pass the label to add_annotation
